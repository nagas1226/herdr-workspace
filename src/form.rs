...
pub fn run() -> i32 {
    ...
    close_popup();
    // `agent start` waits until the agent is ready. Do it after the popup
    // is gone so Save cannot leave the form stuck open.
    for args in agent_starts {
        if let Err(e) = herdr::spawn(&args) {
            eprintln!("herdr-workspace: {e}");
        }
    }
    code
